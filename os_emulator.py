...
def get_all_content():
    ...
    while idx < len(files):
        if files[idx].type == 'directory':
            files = files.__add__(get_directory_content(
                files[idx].path + files[idx].name + '/'))
        idx += 1
    return files
            ...
            with open(os.environ['VFS'], 'w', newline='', encoding='utf-8') as file:
                ...
                for file in all_files:
                    if file.path.find(deleted_path) != 0 and file.path + file.name != deleted_path:
                        writer.writerow(
                            [file.path, file.name, file.type, file.content])
...
def process_command(console: ConsoleEmulator, command: str, args: list | None):
    ...
    if command == 'clear':
        ...
        if len(args) == 1:
            if args[0] == '-h' or args[0] == '--help':
                console.append_text(
                    'Один аргумент - имя создаваемой директории, -h/--help - отобразить помощь по использованию команды\n')
            else:
                if check_directory_name(args[0]) and exist_directory(console.current_directory + '/' + args[0] + '/') is False:
                    ...
                    if check_directory_name(args[0]) is False:
                        console.append_text(
                            f'Имя {args[0]} содержит недопустимые символы. Имя директории может состоять только из символов a-Z, 0-9 и "_"\n', 'error')
                    else:
                        console.append_text(
                            f'Директория {args[0]} уже существует!\n', 'error')
        else:
            console.append_text(
                'mkdir принимает один аргумент - название директории! Для помощи используйте mkdir -h\n', 'error')
    elif command == "rm":
        if len(args) == 1:
            if args[0] == '-h' or args[0] == '--help':
                console.append_text(
                ...
                if exist_directory(console.current_directory + '/' + args[0] + '/'):
                    ...
                    console.append_text(
                        f'Директории {args[0]} не существует!\n', 'error')
        else:
            console.append_text(
                'rm принимает один аргумент - название директории! Для помощи используйте rm -h\n', 'error')
